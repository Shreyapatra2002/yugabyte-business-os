...
export default function Header({ 
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-14 sm:h-16 flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <div className="flex flex-col min-w-0">
              <span className="font-sans font-extrabold text-sm sm:text-base text-slate-100 tracking-tight leading-none group-hover:text-blue-400 transition-colors truncate">
                YugaByte BOS
              </span>
              <span className="hidden sm:block text-[9px] font-mono font-bold tracking-widest text-slate-500 uppercase leading-none mt-1">
                Business OS
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          <motion.button
            ...
            className={`flex items-center gap-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-full text-[10px] sm:text-xs font-semibold tracking-tight border shadow-xs transition-all cursor-pointer ${
              currentView === 'landing'
          ...
          >
            {currentView === 'landing' ? (
              <>
                <span className="hidden sm:inline">Launch Core Dashboard</span>
                <span className="sm:hidden">Dashboard</span>
                <ArrowUpRight size={13} className="animate-pulse shrink-0" />
              </>
            ) : (
              <>
                <Monitor size={13} className="shrink-0" />
                <span className="hidden sm:inline">Log Out to Website</span>
              ...
              {showNotificationMenu && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute right-0 mt-2.5 w-[calc(100vw-1.5rem)] max-w-80 sm:w-80 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl z-40 py-1.5 divide-y divide-slate-800/60 overflow-hidden"
                  >
      ...
      {/* Mobile branch & role strip (dashboard only) */}
      {currentView === 'dashboard' && (
        <motion.div
          initial={{ opacity: 0, height: 0 }}
          animate={{ opacity: 1, height: 'auto' }}
          className="md:hidden border-t border-slate-900/80 bg-slate-950/95 px-3 py-2 flex items-center gap-2 overflow-x-auto"
        >
          <button
            onClick={() => {
              setShowBranchMenu(!showBranchMenu);
            ...
            className="shrink-0 flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-800 text-[10px] text-slate-300"
          >
          ...
          >
            ...
            <span>{userRole.level}</span>
          </button>
        </motion.div>
      )}

      {/* Mobile dropdowns for branch/role (shared with desktop logic) */}
      <AnimatePresence>
        {currentView === 'dashboard' && showBranchMenu && (
          <>
            <div className="fixed inset-0 z-40 md:hidden" onClick={() => setShowBranchMenu(false)} />
            <motion.div
              initial={{ opacity: 0, y: -8 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -8 }}
              className="md:hidden fixed left-3 right-3 top-[calc(3.5rem+env(safe-area-inset-top))] z-50 rounded-xl bg-slate-900 border border-slate-800 shadow-2xl p-1 max-h-64 overflow-y-auto"
            >
              {branches.map((b) => (
                <button
                  key={b.id}
                  ...
                  className={`w-full text-left px-3 py-2.5 rounded-lg text-xs ${
                    activeBranch.id === b.id ? 'bg-blue-600/15 text-blue-400 font-semibold' : 'text-slate-300'
                  }`}
                >
                  {b.name}
                </button>
      ...
      <AnimatePresence>
        {currentView === 'dashboard' && showRoleMenu && (
          <>
            <div className="fixed inset-0 z-40 md:hidden" onClick={() => setShowRoleMenu(false)} />
            <motion.div
              initial={{ opacity: 0, y: -8 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -8 }}
              className="md:hidden fixed left-3 right-3 top-[calc(3.5rem+env(safe-area-inset-top))] z-50 rounded-xl bg-slate-900 border border-slate-800 shadow-2xl p-1"
            >
              {rolesList.map((r) => (
                <button
                  key={r.level}
                  ...
                  className={`w-full text-left px-3 py-2.5 rounded-lg text-xs font-mono ${
                    userRole.level === r.level ? 'bg-orange-500/15 text-orange-400 font-semibold' : 'text-slate-300'
                  }`}
                >
                  {r.level} Role
                </button>
