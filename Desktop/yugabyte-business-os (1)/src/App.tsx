...
import { Sparkles, Plus, X, Briefcase, FileCheck, ClipboardList, CheckCircle, Menu, Home, Users, FileText, Cpu, ShieldAlert, ArrowUp } from 'lucide-react';
import { 
  initialBranches, 
  initialLeads, 
  initialDeals, 
...
export default function App() {
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] bg-slate-950 text-slate-100 flex flex-col font-sans antialiased selection:bg-blue-600/50 selection:text-white overflow-x-hidden">
      {/* Dynamic Header */}
      <Header 
        ...
        <AnimatePresence mode="wait">
          {currentView === 'landing' ? (
            ...
            >
              ...
              <div className="flex-1 min-w-0 bg-slate-950 pb-nav-safe md:pb-0 overflow-x-hidden">
                <DashboardView 
                  activeTab={dashboardTab}
                  setActiveTab={setDashboardTab} 
                  activeBranch={activeBranch}
              ...
              <motion.nav
                ...
                className="fixed md:hidden bottom-0 left-0 right-0 z-30 bg-slate-950/95 backdrop-blur-lg border-t border-slate-800/80 shadow-2xl shadow-black/40 safe-bottom"
              >
                <div className="h-16 px-2 sm:px-4 flex items-stretch justify-between max-w-lg mx-auto">
                  {[
                    ...
                    return (
                      <button
                        key={item.tab}
                        onClick={() => {
                          setDashboardTab(item.tab as SidebarTab);
                          setIsSidebarOpen(false);
                        }}
                        className="relative flex flex-col items-center justify-center flex-1 py-1.5 cursor-pointer min-w-0"
                      >
                        {isSel && (
                          <motion.span
                            layoutId="mobile-nav-pill"
                            className="absolute inset-x-1 top-1 bottom-1 rounded-xl bg-indigo-600/15 border border-indigo-500/20"
                            transition={{ type: 'spring', stiffness: 380, damping: 32 }}
                          />
                        )}
                        <span className={`relative z-10 p-1 rounded-full transition-colors duration-200 ${isSel ? 'text-indigo-400' : 'text-slate-500'}`}>
                          <Icon size={16} />
                        </span>
                        <span className={`relative z-10 text-[8px] sm:text-[9px] font-medium mt-0.5 tracking-tight truncate max-w-full px-0.5 ${isSel ? 'text-indigo-400 font-bold' : 'text-slate-500'}`}>
                          {item.label}
                        </span>
                      </button>
                    );
      ...
      <AnimatePresence>
        {showBackToTop && (
          <motion.button
            ...
            onClick={scrollToTop}
            className="fixed bottom-20 md:bottom-24 right-4 sm:right-6 z-40 bg-slate-900/95 backdrop-blur-md hover:bg-slate-850 text-slate-205 border border-slate-800 hover:border-slate-700/60 p-3.5 rounded-full shadow-2xl shadow-black/80 cursor-pointer transition-all hover:scale-110 active:scale-95 flex items-center justify-center text-blue-400 font-semibold safe-bottom"
      ...
      {currentView === 'dashboard' && (
        <button
          ...
          className="fixed bottom-[calc(4.5rem+env(safe-area-inset-bottom))] md:bottom-6 right-4 sm:right-6 z-40 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-full p-3.5 sm:p-4 shadow-xl shadow-blue-500/20 border border-blue-400/25 cursor-pointer transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-2 group font-semibold font-sans"
